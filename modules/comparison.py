...
def run_single_point_comparison(mf, backend, out_file, molecule="H2"):
    ...
    ecore_hf, h1e_hf, h2e_hf = hamiltonian.get_casci_hamiltonian(
        mf, ncas=ncas_full, nelecas=nelecas_full
    )
    H_qubit_hf = hamiltonian.build_qubit_hamiltonian(
        ecore_hf, h1e_hf, h2e_hf, mapping_method="jordan_wigner"
    )
    ...
    ecore_cas, h1e_cas, h2e_cas = hamiltonian.get_casci_hamiltonian(
        mf, ncas=ncas, nelecas=nelecas
    )
    H_qubit_cas = hamiltonian.build_qubit_hamiltonian(
        ecore_cas, h1e_cas, h2e_cas, mapping_method="jordan_wigner"
    )
    ...
    if molecule == "H2":
        plt.axhline(
            ...
            color="r",
            linestyle="--",
            label=f"Full-CI Energy (Sto-3G) {full_ci_energy:.6f} Ha",
        )
        plt.axhline(
            y=hf_energy,
            color="g",
            linestyle="--",
            label=f"Hartree-Fock Energy (Sto-3G) {hf_energy:.6f} Ha",
        )
...
def initial_parameters_influence(mf, backend, out_file, molecule="H2"):
    ...
    H_qubit = hamiltonian.build_qubit_hamiltonian(
    ...
    n_initial_params = 5
    initial_parameters = [
        list(2 * np.pi * np.random.rand(ansatz.num_parameters))
        for _ in range(n_initial_params)
    ]
